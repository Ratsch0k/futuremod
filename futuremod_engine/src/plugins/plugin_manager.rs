...
impl GlobalPluginManager {
    ...
    where
        F: Fn(&mut PluginManager) -> Result<R, anyhow::Error>,
    {
        let plugin_manager;
        unsafe { plugin_manager = GLOBAL_PLUGIN_MANAGER.get().unwrap() }
        ...
        match plugin_manager.lock() {
            ...
            Err(e) => {
                return Err(anyhow!(
                    "could not get mutable lock to plugin manager: {:?}",
                    e
                ))
            }
        }
    ...
    pub fn initialize(plugins_directory: PathBuf) -> Result<(), anyhow::Error> {
        ...
        unsafe {
            GLOBAL_PLUGIN_MANAGER
                .set(p)
                .map_err(|_| anyhow!("global plugin manager already initialized"))
        }
    }
}
...
fn add_plugin_to_persistence(
    persistence: &mut PersistedPlugins,
    plugin: &Plugin,
...
fn persist_plugin_state_change(
    persistence: &mut PersistedPlugins,
    plugin: &Plugin,
    state: PersistentPluginState,
) {
    debug!(
        "Changing persistence state of plugin {} to {:?}",
        plugin.info.name, state
    );
...
impl PluginManager {
    ...
    pub fn new(plugins_directory: PathBuf) -> Result<Self, PluginManagerError> {
        ...
        if !plugins_directory.is_dir() {
            ...
            if let Err(e) = fs::create_dir_all(&plugins_directory) {
                error!(
                    "Error while creating the plugin directory: {}",
                    e.to_string()
                );
        ...
        let mut persisted_plugins = PersistedPlugins::new(&plugin_states_file)
            .map_err(|e| PluginManagerError::Other(e.to_string()))?;

        info!("Loading plugins from {:?}", plugins_directory);
        let plugin_directories = plugins_directory
            .read_dir()
            .map_err(PluginManagerError::Io)?
            .filter_map(|path| match path {
                Ok(path) => {
                    match path.path().is_dir() || junction::exists(path.path()).unwrap_or(false) {
                        true => Some(path),
                        false => {
        ...
        for plugin_folder in plugin_directories {
            debug!(
                "Discovered plugin folder {}",
                plugin_folder.path().display()
            );
                ...
                None => {
                    ...
                    persisted_plugins
                        .insert(
                            &name,
                            PersistedPlugin {
                                state: PersistentPluginState::Disabled,
                                in_dev_mode: false,
                            },
                        )
                        .map_err(|e| PluginManagerError::Other(e.to_string()))?;
        ...
        info!(
            "Loaded {} plugins, {} errored",
            successfully_loads, errored_loads
        );
        ...
        Ok(PluginManager {
            ...
            plugins_directory,
            lua,
            persistent_states: persisted_plugins,
        })
    }
        ...
        for (_, plugin) in &self.plugins {
            if plugin.is_enabled() {
                ...
                match plugin.on_update() {
                    Err(e) => warn!(
                        "Plugin '{}' main function threw error: {:?}",
                        plugin.info.name, e
                    ),
                ...
                debug!(
                    "Not calling on_update for plugin '{}', plugin not enabled",
                    plugin.info.name
                );
            }
        }
    ...
    pub fn enable_plugin(&mut self, name: &String) -> Result<(), PluginManagerError> {
        ...
        persist_plugin_state_change(
            &mut self.persistent_states,
            plugin,
            PersistentPluginState::Enabled,
        );
    ...
    pub fn install_plugin_from_folder(
        ...
        debug!(
            "Plugin name '{}' sanitized to '{}'",
            plugin_info.name, plugin_folder_name
        );
    ...
    pub fn install_plugin_in_dev_mode(
        &mut self,
        folder: &PathBuf,
    ) -> Result<(), PluginInstallError> {
        info!(
            "Installing plugin in developer mode from '{}'",
            folder.display()
        );
        ...
        let plugin_info =
            load_plugin_info(&folder).map_err(|_| PluginInstallError::InvalidPluginFolder)?;
        ...
        debug!(
            "Creating a softlink from the plugin folder to the destination in the plugins folder"
        );
    ...
    fn add_and_load_plugin_from_folder(
        &mut self,
        folder: &PathBuf,
        in_dev_mode: bool,
    ) -> Result<(), PluginInstallError> {
        ...
        if in_dev_mode {
            ...
            debug!(
                "Plugin installed in dev mode. Adjusting path to '{}'",
        ...
        if self.plugins.contains_key(&plugin_name) {
            info!(
                "Cannot add plugin '{}' since its already installed",
                plugin_name
            );
            return Err(PluginInstallError::AlreadyInstalled);
        }
        ...
        plugin
            .load()
            .map_err(|e| PluginInstallError::Plugin(format!("{:?}", e)))?;
    ...
    pub fn unload_plugin(&mut self, name: &str) -> Result<(), PluginManagerError> {
        ...
        persist_plugin_state_change(
            &mut self.persistent_states,
            &plugin,
            PersistentPluginState::Unloaded,
        );
    ...
    pub fn uninstall_plugin(&mut self, name: &str) -> Result<(), PluginManagerError> {
        ...
        if let Err(e) = plugin.disable() {
            warn!(
                "Plugin {} threw an error while it was disabled: {:?}",
                name, e
            );
        };
        ...
        if let None = self.plugins.remove(name) {
            warn!(
                "Could not find plugin '{}' while removing it from the internal map",
                name
            );
        }
...
fn copy_plugin_directory_to_plugins_folder(
    source: &PathBuf,
    destination: &PathBuf,
) -> Result<(), PluginInstallError> {
    debug!("Copying files from plugin package to destination");
    for file in WalkDir::new(source).into_iter().filter_map(|e| e.ok()) {
        ...
        let relative_path = match path.strip_prefix(source) {
            Ok(v) => v,
            Err(err) => {
                return Err(PluginInstallError::Copy(format!(
                    "Could not get relative path of {}: {}",
                    path.display(),
                    err.to_string()
                )))
            }
        ...
        if path.is_dir() {
            match fs::create_dir_all(&destination_path) {
                Err(err) => {
                    return Err(PluginInstallError::Copy(format!(
                        "Could not destination directory {}: {}",
...
fn softlink_plugin_directory_to_plugins_folder(
    source: &PathBuf,
    destination: &PathBuf,
) -> Result<(), PluginInstallError> {
    junction::create(&source, &destination).map_err(|e| {
        PluginInstallError::IO(format!("Could not create softlink: {}", e).to_string())
    })
}
...
fn delete_plugin_folder(plugin: &Plugin) -> Result<(), PluginManagerError> {
    match plugin.in_dev_mode {
        false => fs::remove_dir_all(&plugin.info.path).map_err(PluginManagerError::Io),
        true => {
