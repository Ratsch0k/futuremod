...
pub fn load_plugin_info(
    path: &PathBuf,
) -> Result<futuremod_data::plugin::PluginInfo, PluginInfoError> {
    let path = path
        .canonicalize()
        .map_err(|e| PluginInfoError::Other(format!("Could not access plugin folder: {:?}", e)))?;
    ...
    let content = match fs::read_to_string(info_file_path) {
        Ok(c) => c,
        Err(e) => {
            return Err(PluginInfoError::Other(format!(
                "Could not read the plugin's info file: {:?}",
                e
            )))
        }
    };

    let plugin_info: futuremod_data::plugin::PluginInfoContent =
        match toml::from_str(content.as_str()) {
            Ok(v) => v,
            Err(e) => {
                return Err(PluginInfoError::Format(format!(
                    "Format of info file is incorrect: {:?}",
                    e
                )))
            }
        };
