...
fn try_userdata_to_bytes(userdata: &mlua::AnyUserData) -> LuaResult<Vec<u8>> {
    userdata.call_method("toBytes", ()).map_err(|e| {
        mlua::Error::RuntimeError(format!("Could not convert userdata into bytes: {}", e))
    })
}
    ...
    let bytes: Vec<u8> = match data {
        ...
        _ => {
            return Err(mlua::Error::RuntimeError(
                "invalid argument. following types are supported: table, number, integer, string"
                    .to_string(),
            ))
        }
    };
    ...
    unsafe {
        value = match value_type {
            ...
            Type::UnsignedInteger => {
                mlua::Value::Integer(TryInto::<i32>::try_into(*(address as *const u32)).unwrap())
