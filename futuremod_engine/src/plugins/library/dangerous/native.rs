        ...
        methods.add_meta_function(
            ...
            |lua,
             ...
             -> Result<mlua::Value, mlua::Error> {
                ...
                debug!(
                    "Getting field '{}' from native struct at 0x{:x}",
                    field, native_struct.address
                );
                let native_field = match native_struct.fields.get(&field) {
                    Some(field) => field,
                    None => {
                        debug!(
                            "Native struct at 0x{:x} doesn't have field {}",
                            native_struct.address, field
                        );
                        return Ok(mlua::Nil);
                    }
                };
                ...
                match &native_field.field_type {
                    FieldType::Primitive(primitive) => unsafe {
                        let value = *(field_ptr as *const u32);
                        native_to_lua(lua, *primitive, value)
                    },
                    FieldType::Complex(complex) => {
                        let complex_type: AnyUserData = native_struct_userdata
                            .named_user_value(&complex)
                            .map_err(|e| {
            ...
            |_,
             ...
             -> Result<(), mlua::Error> {
                ...
                debug!(
                    ...
                    field, native_struct.address, value
                );
                ...
                let native_field = match native_struct.fields.get(&field) {
                    ...
                    None => {
                        debug!(
                            "Struct at 0x{:x} doesn't have field {}",
                            native_struct.address, field
                        );
                ...
                match &native_field.field_type {
                    FieldType::Primitive(primitive) => {
                        let native_value = unsafe {
                            lua_to_native(*primitive, &value).map_err(|e| {
                                mlua::Error::RuntimeError(format!(
                                    "Could not convert lua value into native: {}",
                                    e
                                ))
                            })?
                        };
                        ...
                        if native_value.len() > 1 {
                            debug!(
                                "Converted lua value is larger than one double word: {:?}",
                                value
                            );
                        }
...
pub fn create_native_struct_definition_fn(
        ...
        let (key, field_definition) = match pair {
            ...
            Err(e) => {
                ...
                return Err(mlua::Error::RuntimeError(format!(
                    "Field definition must be a table: {}",
                    e
                )));
            }
        };

        let offset: u32 = field_definition.get("offset").map_err(|_| {
            mlua::Error::RuntimeError(format!("Field definition of {} is missing 'offset'", key))
        })?;
        let native_type_id: mlua::Value = field_definition.get("type").map_err(|_| {
            mlua::Error::RuntimeError(format!("Field definition of {} is missing 'type'", key))
        })?;
            ...
            "userdata" => match native_type_id.as_userdata() {
                ...
                None => {
                    return Err(mlua::Error::runtime(
                        "Could not convert type userdata to userdata",
                    ))
                }
            },
    ...
    let definition_userdata = lua.create_userdata(NativeStructDefinition {
