...
use futuremod_hook::lua::{create_native_function_function, get_native_function};
use mlua::Lua;
...
pub fn create_dangerous_library(lua: Arc<Lua>) -> Result<mlua::OwnedTable, mlua::Error> {
    ...
    let create_native_struct_definition =
        lua.create_function(create_native_struct_definition_fn)?;
    table.set(
        "createNativeStructDefinition",
        create_native_struct_definition,
    )?;
