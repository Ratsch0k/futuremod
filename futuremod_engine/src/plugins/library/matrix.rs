...
pub fn create_matrix_library(lua: Arc<Lua>) -> Result<OwnedTable, mlua::Error> {
    ...
    table.set(
        "zerosFloat",
        lua.create_function(create_zero_matrix::<f32>)?,
    )?;
    table.set(
        "identityFloat",
        lua.create_function(create_identity_matrix::<f32>)?,
    )?;
    ...
    table.set(
        "identityInt",
        lua.create_function(create_identity_matrix::<i32>)?,
    )?;
...
trait MatrixContext<M>: HasArc<M> {
    ...
    fn with_matrix<F, R>(&self, f: F) -> LuaResult<R>
    where
        F: Fn(&M) -> LuaResult<R>,
    {
        match self.get_arc().lock() {
            Ok(matrix) => f(&matrix),
            Err(_) => Err(mlua::Error::RuntimeError("Matrix is locked".to_string())),
        }
    }

    /// Call the given function with a mutable reference to the matrix.
    fn with_matrix_mut<F, R>(&self, f: F) -> LuaResult<R>
    where
        F: Fn(&mut M) -> LuaResult<R>,
    {
        match self.get_arc().lock() {
            Ok(mut matrix) => f(&mut matrix),
            Err(_) => Err(mlua::Error::RuntimeError("Matrix is locked".to_string())),
        }
    }
}
    ...
    > UserData for LuaMatrix<T>
{
    fn add_fields<'lua, F: mlua::UserDataFields<'lua, Self>>(fields: &mut F) {
        ...
        fields.add_field_method_get("nrows", |_, matrix| {
            matrix.with_matrix(|matrix| Ok(matrix.nrows()))
        });
    }
        ...
        M: UserDataMethods<'lua, LuaMatrix<T>>,
    {
        methods.add_method("at", |_, matrix, (row, col): (u8, u8)| -> LuaResult<T> {
            ...
            matrix.with_matrix(|matrix| Ok(matrix[(row as usize, col as usize)]))
        });

        methods.add_method_mut(
            "set",
            |_, matrix, (row, col, value): (u8, u8, T)| -> LuaResult<()> {
                matrix.check_bounds(row, col)?;
        ...
        methods.add_meta_method(
            MetaMethod::Add,
            |_, matrix, rhs: LuaMatrix<T>| -> LuaResult<LuaMatrix<T>> {
                let result = matrix.with_matrix(|lhs| rhs.with_matrix(|rhs| Ok(lhs + rhs)))?;
...
fn create_matrix<'lua, T: Scalar>(_: &'lua Lua, data: Vec<Vec<T>>) -> LuaResult<LuaMatrix<T>> {
    ...
    Ok(LuaMatrix(Arc::new(Mutex::new(
        DMatrix::<T>::from_row_slice(rows, cols, &data),
    ))))
}
    ...
    > UserData for MatrixType<T>
{
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method("getByteSize", |_, this, ()| Ok(4 * this.nrows * this.ncols));

        methods.add_method("toBytes", |_, this, matrix: LuaMatrix<T>| {
            let matrix_arc = matrix.get_arc();
            let inner_matrix = matrix_arc.lock().map_err(|_| {
                mlua::Error::RuntimeError("Could not get lock to matrix".to_string())
            })?;
        ...
        methods.add_method(
            "fromBytes",
            |lua, this, bytes: Vec<u8>| -> LuaResult<LuaMatrix<T>> {
                ...
                info!(
                    "Expecting {} bytes, each value has size {}",
                    byte_size, value_byte_size
                );

                if bytes.len() < byte_size {
                    return Err(mlua::Error::RuntimeError(format!(
                        "{} bytes required to construct the matrix",
                        byte_size
                    )));
                }
                ...
                for row_idx in 0..this.nrows as usize {
                    ...
                    for col_idx in 0..this.ncols as usize {
                        matrix_data[row_idx as usize]
                            .push(values[row_idx * this.ncols as usize + col_idx]);
                        matrix_str.push_str(
                            format!("{:?} ", values[row_idx * this.ncols as usize + col_idx])
                                .as_str(),
                        );
                    }
                    matrix_str.push_str("\n");
...
impl UserData for ModelMatrix {
    ...
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method_mut(
            "set",
            |_, matrix, (row, col, value): (u8, u8, f32)| -> LuaResult<()> {
                matrix.check_bounds(row, col)?;
        ...
        methods.add_meta_method(
            MetaMethod::Add,
            |_, matrix, rhs: ModelMatrix| -> LuaResult<ModelMatrix> {
                let result = matrix.with_matrix(|lhs| rhs.with_matrix(|rhs| Ok(lhs + rhs)))?;
        ...
        methods.add_function("fromBytes", |_, bytes: Vec<u8>| -> LuaResult<ModelMatrix> {
            if bytes.len() < 64 {
                return Err(mlua::Error::RuntimeError(
                    "Model matrix requires 64 bytes".to_string(),
                ));
            }
        ...
        methods.add_method(
            "rotate",
            |_, matrix, (x, y, z, angle): (f32, f32, f32, f32)| {
                matrix.with_matrix_mut(|matrix| {
