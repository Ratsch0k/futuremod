...
impl UserData for PlayerEntity {
    fn add_fields<F: mlua::prelude::LuaUserDataFields<Self>>(fields: &mut F) {
        ...
        fields.add_field_method_get("positionX", |_, this| {
            Ok(unsafe { (*this.player_entity).position_x })
        });
        ...
        fields.add_field_method_get("positionY", |_, this| {
            Ok(unsafe { (*this.player_entity).position_y })
        });
        ...
        fields.add_field_method_get("positionZ", |_, this| {
            Ok(unsafe { (*this.player_entity).position_z })
        });
        ...
        create_getter_setter("deaths", fields, |player| unsafe {
            &mut (*(*player).player).deaths
        });
        create_getter_setter("currentAction", fields, |player| unsafe {
            &mut (*(*player).player).current_action
        });
        create_getter_setter("movementMode", fields, |player| unsafe {
            &mut (*(*player).player).movement_mode
        });
        create_getter_setter("currentTargetType", fields, |player| unsafe {
            &mut (*(*player).player).current_target_type
        });
        create_getter_setter("currentTarget", fields, |player| unsafe {
            &mut (*(*player).player).current_target
        });
        create_getter_setter("lastTarget", fields, |player| unsafe {
            &mut (*(*player).player).last_target
        });
        create_getter_setter("gunWeaponTimeout", fields, |player| unsafe {
            &mut (*(*player).player).gun_weapon_timeout
        });
        create_getter_setter("heavyWeaponTimeout", fields, |player| unsafe {
            &mut (*(*player).player).heavy_weapon_ammo
        });
        create_getter_setter("specialWeaponTimeout", fields, |player| unsafe {
            &mut (*(*player).player).special_weapon_timeout
        });
        create_getter_setter("gunWeaponAmmo", fields, |player| unsafe {
            &mut (*(*player).player).gun_weapon_ammo
        });
        create_getter_setter("heavyWeaponAmmo", fields, |player| unsafe {
            &mut (*(*player).player).heavy_weapon_ammo
        });
        create_getter_setter("specialWeaponAmmo", fields, |player| unsafe {
            &mut (*(*player).player).special_weapon_ammo
        });
        create_getter_setter("selectedGunWeapon", fields, |player| unsafe {
            &mut (*(*player).player).selected_gun_weapon
        });
        create_getter_setter("selectedHeavyWeapon", fields, |player| unsafe {
            &mut (*(*player).player).selected_heavy_weapon
        });
        create_getter_setter("selectedSpecialWeapon", fields, |player| unsafe {
            &mut (*(*player).player).selected_special_weapon
        });
        create_getter_setter("playerNumber", fields, |player| unsafe {
            &mut (*(*player).player).player_number
        });
        create_getter_setter("accelerationX", fields, |player| unsafe {
            &mut (*(*player).player).acceleration_x
        });
        create_getter_setter("accelerationZ", fields, |player| unsafe {
            &mut (*(*player).player).acceleration_z
        });
        create_getter_setter("accelerationY", fields, |player| unsafe {
            &mut (*(*player).player).acceleration_y
        });
    }
    ...
    let get_player = lua.create_function(|_, player: u8| {
        ...
        if player > 1 {
            return Err(mlua::Error::RuntimeError(
                "Can only get player one or two at the moment.".into(),
            ));
        }
        ...
        unsafe {
            player_array_item =
                *((PLAYER_ARRAY_ADDR + Into::<u32>::into(player) * 8) as *const u32);
        }
