...
use crate::{
    api::{self, get_plugins, reload_plugin},
    util::{get_plugin_info_of_local_folder, is_plugin_folder},
    view::{
        self,
        dashboard::view::{Dialog, InstallConfirmationPrompt},
        logs,
    },
};
...
pub fn update(dashboard: &mut Dashboard, message: Message) -> Task<Message> {
    ...
    match message {
        ...
        | Message::PluginList(view::plugin_list::Message::Enable(plugin)) => {
            return Task::perform(
                async move { api::enable_plugin(plugin).await.map_err(|e| e.to_string()) },
        ...
        | Message::PluginList(view::plugin_list::Message::Disable(plugin)) => {
            return Task::perform(
                async { api::disable_plugin(plugin).await.map_err(|e| e.to_string()) },
        ...
        Message::EnableResponse(response) => match response {
            ...
            Err(e) => {
                dashboard.dialog = Some(Dialog::Error(
                    format!("Could not enable the plugin: {}", e).to_string(),
                ));
        ...
        Message::DisableResponse(response) => match response {
            ...
            Err(e) => {
                dashboard.dialog = Some(Dialog::Error(
                    format!("Could not disable the plugin: {}", e).to_string(),
                ));
                warn!("Could not disable plugin: {}", e);
            }
        },
        Message::GetPluginsResponse(response) => match response {
            Ok(plugins) => {
        ...
        Message::Uninstall(name) => {
            return Task::perform(
                async move { api::uninstall_plugin(name).await.map_err(|e| e.to_string()) },
        ...
        Message::UninstallResponse(response) => {
            match response {
                Ok(()) => {
                    ...
                    return Task::perform(get_plugins(), Message::GetPluginsResponse).chain(
                        Task::batch([
                ...
                Err(e) => {
                    ...
                    dashboard.dialog = Some(Dialog::Error(
                        format!("Could not uninstall the plugin: {}", e).to_string(),
                    ));
                }
            }
        }
        Message::Reload(name) | Message::Plugin(view::plugin::Message::Reload(name)) => {
            return Task::perform(
                async move { reload_plugin(&name).await.map_err(|e| format!("{}", e)) },
                Message::ReloadResponse,
            );
        }
        Message::ReloadResponse(response) => match response {
            Ok(()) => return Task::perform(get_plugins(), Message::GetPluginsResponse),
            Err(e) => {
                warn!("Could not reload plugin: {}", e);
                dashboard.dialog = Some(Dialog::Error(
                    format!("Could not reload the plugin: {}", e).to_string(),
                ));
            }
        },
        ...
        Message::PluginList(view::plugin_list::Message::Install) => {
            ...
            info!(
                "Get plugin info of plugin package at '{}'",
                plugin_package.display()
            );

            return Task::perform(
                async {
                    let response = api::get_plugin_info(plugin_package.clone())
                        .await
        ...
        Message::PluginList(view::plugin_list::Message::DevInstall) => {
            ...
            return Task::perform(
                async move {
                    let response = get_plugin_info_of_local_folder(&plugin_package)
                        .map_err(|e| e.to_string())?;
        ...
        Message::OpenInstallConfirmationPromptDialog(prompt_response) => match prompt_response {
            Ok(prompt) => {
            ...
            Err(e) => {
                ...
                dashboard.dialog = Some(Dialog::Error(
                    format!("Could not fetch plugin information: {}", e).to_string(),
                ));
            }
        },
        Message::ConfirmInstallation(confirmed_prompt) => {
            info!(
                "Install plugin package at '{}'",
                confirmed_prompt.path.display()
            );

            return Task::perform(
                async move {
                    match confirmed_prompt.in_developer_mode {
                        false => api::install_plugin(&confirmed_prompt.path)
                            .await
                            .map_err(|e| e.to_string()),
                        true => api::install_plugin_in_developer_mode(&confirmed_prompt.path)
                            .await
                            .map_err(|e| e.to_string()),
                    }
                },
        ...
        Message::InstallResponse(response) => match response {
            Ok(()) => {
                return Task::perform(
                    async { get_plugins().await.map_err(|e| e.to_string()) },
                    Message::InstallGetPlugins,
                );
            }
            Err(e) => {
                warn!("Could not install plugin: {}", e);
                dashboard.dialog = Some(Dialog::Error(
                    format!("Could not install plugin: {}", e).to_string(),
                ));
            }
        },
        ...
        Message::ToggleSidebar => {
            dashboard
                .sidebar_minimized
                .transition(!dashboard.sidebar_minimized.value, Instant::now());
        }
        Message::ToSettings => {
        ...
        message => match &mut dashboard.view {
            ...
            View::PluginList(plugin_list_view) => match message {
                Message::PluginList(plugin_list_message) => {
                    return plugin_list_view
                        .update(plugin_list_message)
            ...
            View::Debug(debug_view) => match message {
                Message::Debug(debug_message) => {
                    return debug_view.update(debug_message).map(Message::Debug)
                }
                _ => (),
            },
        },
