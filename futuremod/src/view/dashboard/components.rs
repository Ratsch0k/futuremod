...
use iced::{
    ...
    theme::Palette,
    widget::{
        center, column, container, markdown, mouse_area, opaque, row, rule, scrollable, text,
        Space, Stack,
    },
...
use crate::{
    palette::Shade,
    theme::{self, Container, Text, Theme},
    widget::{bold, button, icon_button, icon_with_size, Column, Element, Row},
};

use super::{
    view::{Dialog, InstallConfirmationPrompt, View},
    Dashboard, Message,
};
...
pub fn dashboard<'a>(state: &'a Dashboard) -> Element<'a, Message> {
    let content = match &state.view {
        ...
        View::Plugin(plugin_view) => match state.plugins.get(&plugin_view.name) {
            Some(plugin) => plugin_view.view(&plugin).map(Message::Plugin),
            None => error_box("Plugin doesn't exist".to_string()),
        },
        View::PluginList(plugin_list_view) => plugin_list_view
            .view(&state.plugins, state.is_developer)
    ...
    if let Some(active_dialog) = &state.dialog {
        overlay = Some(opaque(
            mouse_area(center(opaque(dialog(active_dialog))).class(Container::Backdrop))
...
fn sidebar<'a>(
    active_view: &'a View,
    minimized: &'a Animated<bool, Instant>,
) -> Element<'a, Message> {
    container(tabs(active_view, &minimized))
        .padding(16)
...
fn uninstall_prompt<'a>(plugin_name: String) -> Element<'a, Message> {
    container(column![
        ...
        row![
            ...
            button("Uninstall")
                .on_press(Message::Uninstall(plugin_name))
...
fn heading<'a>(is_developer: bool) -> Element<'a, Message> {
    row![container(title(is_developer))
        .width(Length::Fill)
        .padding(Padding {
            top: 0.0,
            right: 0.0,
...
fn tabs<'a>(active_view: &View, minimized: &Animated<bool, Instant>) -> Element<'a, Message> {
    let is_plugin_tab = |active_view: &View| match active_view {
        View::PluginList(_) | View::Plugin(_) => true,
        _ => false,
    };

    let tab_button = |icon_content: Bootstrap,
                      label: &'static str,
                      on_press: Option<Message>,
                      active: bool|
     -> Element<'_, Message> {
        let mut content = Row::with_children([icon_with_size(icon_content, 16).into()]).clip(false);

        let alpha = minimized.animate_bool(1.0, 0.0, Instant::now());

        if alpha > 0.1 {
            content = content.push(
                container(text(label).wrapping(text::Wrapping::None).height(20).class(
                    theme::Text::Custom(Box::new(move |theme| text::Style {
                        color: Some(Color {
                            a: alpha,
    ...
    let mut buttons = column![
        tab_button(
            if !minimized.value {
                Bootstrap::ChevronDoubleLeft
            } else {
                Bootstrap::ChevronDoubleRight
            },
            "Minimize",
        ...
        tab_button(
            Bootstrap::Box,
            "Plugins",
            Some(Message::ToPluginList),
            is_plugin_tab(&active_view)
        ),
        tab_button(
            Bootstrap::CardText,
            "Logs",
            Some(Message::ToLogs),
            matches!(active_view, View::Logs(_))
        ),
    ];

    #[cfg(feature = "debug")]
    {
        buttons = buttons.push_maybe(if cfg!(feature = "debug") {
            Some(tab_button(
                Bootstrap::Bug,
                "Debug",
                Some(Message::ToDebug),
                matches!(active_view, View::Debug(_)),
            ))
        } else {
            None
        });
    ...
    buttons
        ...
        .push(tab_button(
            Bootstrap::Gear,
            "Settings",
            Some(Message::ToSettings),
            matches!(active_view, View::Settings(_)),
        ))
        .spacing(8.0)
        .into()
}
...
fn installation_prompt<'a>(
    confirmation_prompt: &'a InstallConfirmationPrompt,
) -> Element<'a, Message> {
    let warning: Option<iced::widget::Container<Message, Theme>> = if confirmation_prompt
        .plugin
        .dependencies
        .contains(&PluginDependency::Dangerous)
    {
        Some(
      container(
        ...
        container(column![
            ...
            container(scrollable(
                Column::new()
                    .push(text(format!(
                        ...
                        confirmation_prompt.plugin.name.clone()
                    )))
                    .push_maybe(warning)
                    .push(info_box(
                        "General Information",
                        column![
                            ...
                            text(format!(
                                "Authors: {}",
                                confirmation_prompt.plugin.authors.clone().join(", ")
                            )),
                    ...
                    .push(info_box(
                        ...
                        if confirmation_prompt.plugin.description.len() <= 0 {
                            ...
                            markdown::view(
                                &confirmation_prompt.parsed_description,
                                markdown::Settings::default(),
                                markdown::Style::from_palette(Palette::DARK),
                            )
                            .map(Message::OpenUrl)
                            .into()
                        }
            ...
            row![
                ...
                button(text("Cancel"))
                    ...
                    .on_press(Message::CloseDialog),
                button(text("Install"))
                    .on_press(Message::ConfirmInstallation(confirmation_prompt.clone()))
