...
use crate::{
    theme,
    widget::{
        bold, button, icon_text_button, icon_text_button_advanced, Column, Element,
...
pub fn plugin_overview<'a>(
    plugins: &'a HashMap<String, Plugin>,
    is_developer: bool,
) -> Element<'a, Message> {
    column![plugin_overview_heading(is_developer), plugin_list(plugins),]
        .spacing(16)
        .padding(16)
        .into()
}
...
fn plugin_overview_actions<'a>(is_developer: bool) -> Element<'a, Message> {
    ...
    actions = actions.push(
        icon_text_button_advanced(
            Bootstrap::Plus,
            "Install",
            IconTextButtonOptions::new().with_icon_size(24),
        )
...
fn plugin_card_description<'a>(plugin: &Plugin) -> Element<'a, Message> {
    let dev_mode_badge: Option<Element<'a, Message>> = match &plugin.in_dev_mode {
        false => None,
        true => Some(
            Badge::new(text("Developer Mode").font(bold()).size(12))
                .style(|theme: &theme::Theme, status| {
                    iced_aw::style::badge::warning(&theme.theme, status)
                })
                .padding(4)
                .into(),
        ),
