...
use iced::{
    ...
    widget::{
        ...
        scrollable::{Direction, Scrollable, Scrollbar},
        text,
    },
    Length, Renderer, Task,
};
use iced_aw::{
    menu::{Item, Menu},
    menu_bar, menu_items,
...
use crate::{
    api::get_plugins,
    logs::{self, state::LogState, subscriber::LogRecord},
    theme::{Button, Theme},
    widget::bold,
};
use crate::{
...
impl Logs {
    ...
    pub fn view<'a>(&self, log: &'a logs::state::Logs) -> Element<'a, Message> {
        match self {
            Logs::Loading => container(text("Loading..."))
                .height(Length::Fill)
                .width(Length::Fill)
            ...
            Logs::View(loaded_logs) => {
                    ...
                    LogState::Connected => {
                        ...
                        for message in &filtered[start..end] {
                            let origin_text = match &message.plugin {
                                Some(plugin) => text(format!("[{}]", plugin)).font(bold()),
                                None => text(message.target.replace("futuremod_engine::", "")),
                            };
                            ...
                            let time_text = text(
                                message
                                    .timestamp
                                    .parse::<DateTime<Utc>>()
                                    .map_or(message.timestamp.clone(), |v| {
                        ...
                        Scrollable::new(column(lines).padding([0.0, 8.0]))
                ...
                container(column![
                    header(
                        loaded_logs.unlimited_history,
                        &loaded_logs.selected_log_levels,
                        &loaded_logs.plugins,
                        &loaded_logs.selected_origins
                    ),
                    content,
                ])
                .into()
            }
            Logs::Error(msg) => container(text(format!("Could not load plugins: {}", msg)))
                .height(Length::Fill)
                .width(Length::Fill)
                .align_x(Horizontal::Center)
                .align_y(Vertical::Center)
...
fn header<'a>(
    unlimited_history: bool,
    selected_levels: &SelectedLogLevels,
    plugins: &HashMap<String, Plugin>,
    selected_origins: &HashMap<LogOrigin, bool>,
) -> Element<'a, Message> {
    row![
...
fn level_picker<'a>(log_levels: &SelectedLogLevels) -> Element<'a, Message> {
    let filter_button = button("Log Level")
        .on_press(Message::None)
        .class(Button::Text);

    menu_bar!((filter_button, {
        Menu::new(menu_items!((checkbox("Debug", log_levels.debug)
            .on_toggle(Message::ToggleLevelDebug)
            .width(Length::Fill))(
            checkbox("Info", log_levels.info)
                .on_toggle(Message::ToggleLevelInfo)
                .width(Length::Fill)
        )(
            checkbox("Warn", log_levels.warn)
                .on_toggle(Message::ToggleLevelWarn)
                .width(Length::Fill)
        )(
            checkbox("Error", log_levels.error)
                .on_toggle(Message::ToggleLevelError)
                .width(Length::Fill)
        )))
        .spacing(8.0)
        .width(140.0)
...
fn origin_picker<'a>(
    ...
    menu_bar!((filter_button, {
        Menu::new(items).spacing(8.0).width(148.0)
    }))
    .into()
}
