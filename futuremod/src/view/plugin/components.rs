...
use crate::{
    palette::Shade,
    theme::{self, Button, Container, Text},
    widget::{
        bold, button, icon, icon_text_button, icon_text_button_advanced, Column, Element,
        IconTextButtonOptions, Row,
    },
};
...
fn plugin_uninstall_button<'a>(plugin: &Plugin) -> Element<'a, Message> {
    icon_text_button_advanced(
        Bootstrap::X,
        "Uninstall",
        IconTextButtonOptions::default().with_icon_size(24),
    )
    .padding(Padding {
        top: 3.0,
        right: 16.0,
...
pub fn plugin_details_view<'a>(
    plugin_view: &'a super::Plugin,
    plugin: &Plugin,
    show_reload_success_msg: bool,
) -> Element<'a, Message> {
    ...
    column![
        container(column![
            row![
                button(icon(Bootstrap::ArrowLeft))
                    .class(Button::Text)
...
fn plugin_details_content<'a>(
    description: &'a Vec<markdown::Item>,
    plugin: &Plugin,
) -> Element<'a, Message> {
    ...
    Scrollable::new(
        column![
            plugin_info_box(column![box_header("Description"), description,].spacing(8.0)),
            plugin_info_box(
                column![
