...
use crate::{
    api::{self, is_mod_running},
    config,
    injector::{get_future_cop_handle, inject_mod},
    theme,
    widget::{button, icon_with_size, Element},
};
...
impl Loading {
    pub fn new() -> (Self, Task<Message>) {
        ...
        match mod_path.exists() {
            true => {
                ...
                (
                    Loading::WaitingForProgram {
                        mod_path,
                        error: None,
                        settings: None,
                    },
                    Task::perform(async {}, |_| Message::CheckIfStarted),
                )
            }
            false => {
    ...
    pub fn view(&self) -> Element<Message> {
        let content: Column<Message, theme::Theme> = match self {
            Loading::WaitingForProgram {
                ...
                column![
                    text("Waiting for program to start").size(24),
                    button("Change Settings").on_press(Message::OpenSettings),
                ]
                .spacing(16)
            ...
            Loading::InjectionError { error, .. } => column![
                text(error),
                button("Retry").on_press(Message::CheckIfStarted),
            ]
            .into(),
            Loading::NoPath => column![
                text("Mod Not Found").size(24),
                text("Could not find mod, please select the mod"),
                button("SELECT").on_press(Message::OpenPathSelection),
            ]
            .into(),
            Loading::FetchingPlugins => column![text("Fetching plugins")].into(),
    ...
    pub fn update(&mut self, msg: Message) -> Task<Message> {
        match self {
            Loading::WaitingForProgram {
                ...
                Message::ChangeModPath(path) => {
                    let path_str = match path.to_str() {
                        ...
                        None => {
                            *error =
                                Some(String::from("Invalid path. Please select another file."));
                            return Task::none();
                        }
                    };
                    ...
                    *self = Loading::WaitingForProgram {
                        mod_path: path,
                        error: None,
                        settings: None,
                    };
            ...
            Loading::WaitingForMod {
                since,
                injection_attempts: injection_tries,
                mod_path,
                settings,
            } => {
                match msg {
                    ...
                    Message::IsModActive(is_active) => match is_active {
                        true => {
                            *self = Loading::FetchingPlugins;
                            ...
                            return Task::perform(
                                async { api::get_plugins().await.map_err(|e| e.to_string()) },
                        ...
                        false => {
                            ...
                            if now.duration_since(*since).unwrap()
                                > Duration::from_secs(INJECTION_WAIT_TIMEOUT_SECONDS)
                            {
                                ...
                                if *injection_tries >= MAX_INJECTION_TRIES {
                                    ...
                                    *self = Loading::InjectionError {
                                        mod_path: mod_path.clone().to_path_buf(),
                                        error: String::from("Was not able to inject the mod"),
                                    };
                                    return Task::none();
                                }
                                ...
                                *self = Loading::WaitingForMod {
                                    ...
                                    injection_attempts: *injection_tries + 1,
    ...
    fn try_to_inject_mod(&mut self, mod_path: PathBuf) -> Task<Message> {
        ...
        match get_future_cop_handle(config.require_admin) {
            ...
            Err(e) => {
                warn!(
                    "Error while trying to the a handle to the FutureCop process: {}",
                    e
                );
            }
        }
