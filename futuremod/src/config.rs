...
use std::{
    ...
    fs, io,
    path::{Path, PathBuf},
    sync::{Arc, LazyLock, OnceLock, RwLock, RwLockReadGuard},
};
...
fn default_mod_path() -> String {
    ...
    let current_dir = current_dir_path
        .to_str()
        .expect("Could not convert the path to the current directory to a string");
...
fn get_config_from_path(path: &Path) -> Result<Config, anyhow::Error> {
    if path.exists() {
        ...
        let config_content =
            fs::read_to_string(path).map_err(|e| anyhow!("Could not read the config: {}", e))?;
...
pub static DEFAULT_CONFIG: LazyLock<Config> =
    LazyLock::new(|| create_default_config().expect("Could not create default config"));
...
pub fn get<'a>() -> RwLockReadGuard<'a, Config> {
    ...
    config
        .config
        .read()
        .expect("Cannot read config, it is locked")
}
...
pub fn update(f: impl Fn(&mut Config) -> ()) -> Result<(), ConfigWriteError> {
    ...
    let mut writable_config = config
        .config
        .write()
        .expect("Cannot write to config, it is locked");

    // Let the provided function update the config
    f(&mut writable_config);

    // Write the changed config to the config file
    let config_content = serde_json::to_string::<Config>(&writable_config)
        .map_err(ConfigWriteError::Serialization)?;
