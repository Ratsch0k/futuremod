...
pub async fn install_plugin(path: &PathBuf) -> Result<(), anyhow::Error> {
    let file = fs::File::open(path.clone())
        .await
        .map_err(|e| anyhow!("Could not open file: {}", e.to_string()))?;
...
pub async fn install_plugin_in_developer_mode(path: &PathBuf) -> Result<(), anyhow::Error> {
    ...
    let path_str = path
        .to_str()
        .ok_or(anyhow!("Could not convert folder path to string"))?;
...
pub async fn get_plugin_info(path: PathBuf) -> Result<PluginInfo, anyhow::Error> {
    ...
    if !response.status().is_success() {
        ...
        let err = if err.len() <= 0 { entire_response } else { err };
...
pub async fn uninstall_plugin(name: String) -> Result<(), anyhow::Error> {
    ...
    let _ = reqwest::Client::new()
        ...
        .map_err(|e| {
            anyhow!(
                "Could not send request to uninstall plugin: {}",
                e.to_string()
            )
        })?
        .error_for_status()
...
pub async fn parse_json<T>(response: reqwest::Response) -> Result<T, String>
where
    T: DeserializeOwned,
{
