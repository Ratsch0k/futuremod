...
lazy_static! {
    static ref HOOKS: Arc<Mutex<HashMap<u32, Arc<Mutex<Inner>>>>> =
...
pub unsafe fn install_hook<Fn>(target_fn_address: usize, hook_fn: Fn) -> Option<Fn> {
    ...
    memory_copy(
        target_fn_address as *const c_void as u32,
        trampoline as u32,
        prelude_size as u32,
    );
    ...
    *trampoline_jmp_address = 0xe9u8;
    ...
    memory_copy(
        &trampoline_delta as *const isize as *const u8 as u32,
        (trampoline as usize + prelude_size as usize + 1) as *mut u8 as u32,
        4,
    );
    ...
    VirtualProtect(
        target_fn_address as *const c_void,
        1024,
        PAGE_EXECUTE_READWRITE,
    ...
    *target_jmp_address = 0xe9;
    memory_copy(
        &jmp_delta as *const isize as *const u8 as u32,
        (target_fn_address as usize + 1) as *mut isize as *mut u8 as u32,
        4,
    );
...
impl Hook {
    ...
    pub unsafe fn set_closure<T: ?Sized>(&mut self, closure: Box<T>) -> Result<(), HookError> {
        let mut inner = self
            .inner
            .lock()
            .map_err(|e| HookError::Other(format!("{}", e)))?;
        ...
        // Keep track of allocated memory sections
        let mut allocated_sections: Vec<u32> = Vec::new();

        // Allocate memory to hold the trampoline
        // The trampoline will contain the first prelude_size bytes from the target function and
        // 5 additional bytes to jump to the original function
        let target_trampoline = VirtualAlloc(
            None,
            trampoline_size,
            MEM_COMMIT | MEM_RESERVE,
            PAGE_EXECUTE_READWRITE,
        ...
        let patched_prelude =
            get_patched_prelude(inner.address, required_bytes, target_trampoline as u32)?;
        ...
        let target_trampoline_delta =
            target_trampoline_dst as isize - target_trampoline_src as isize;
        ...
        *target_trampoline_jmp_address = 0xe9u8;
        ...
        // Write the jump address into the trampoline
        memory_copy(
            &target_trampoline_delta as *const isize as *const u8 as u32,
            (target_trampoline as usize + prelude_size as usize + 1) as *mut u8 as u32,
            4,
        );
        ...
        // ret
        let hook_trampoline =
            VirtualAlloc(None, 50, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
        allocated_sections.push(hook_trampoline as u32);

        let hook_trampoline_first: [u8; 23] = [
            0x53, 0x89, 0xe3, 0x83, 0xc3, 0x04, 0x89, 0xe0, 0x05, 0xc8, 0x00, 0x00, 0x00, 0xff,
            0x30, 0x83, 0xe8, 0x04, 0x39, 0xd8, 0x7f, 0xf7, 0x68,
        ];
        ...
        memory_copy(
            &hook_trampoline_jump_address as *const u32 as u32,
            hook_trampoline.add(current_offset) as *mut u32 as u32,
            4,
        );
        current_offset += 4;
        ...
        *push_data_instruction_address = 0x68;
        current_offset += 1;
        memory_copy(
            &data as *const u32 as u32,
            hook_trampoline.add(current_offset) as u32,
            4,
        );
        current_offset += 4;
        ...
        let hook_trampoline_jump_delta =
            hook_trampoline_jump_dst as isize - hook_trampoline_jump_src as isize;

        memory_copy(
            &hook_trampoline_jump_delta as *const isize as u32,
            hook_trampoline.add(current_offset) as u32,
            4,
        );
        ...
        *target_jmp_address = 0xe9;
        memory_copy(
            &jmp_delta as *const isize as u32,
            (inner.address as usize + 1) as u32,
            4,
        );
    ...
    pub unsafe fn stack_aware_set_hook(&mut self, hook_fn: u32) -> Result<(), HookError> {
        let mut inner = self
            .inner
            .lock()
            .map_err(|e| HookError::Other(format!("{}", e.to_string())))?;
        ...
        *target_trampoline_jmp_address = 0xe9u8;
        ...
        // Write the jump address into the trampoline
        memory_copy(
            &target_trampoline_delta as *const isize as u32,
            (target_trampoline as usize + prelude_size as usize + 1) as u32,
            4,
        );
        ...
        let hook_trampoline_jump_dst = hook_fn;
        let hook_trampoline_jump_src = hook_trampoline.add(current_offset + 4);
        let hook_trampoline_jump_delta =
            hook_trampoline_jump_dst as isize - hook_trampoline_jump_src as isize;
    ...
    pub unsafe fn unhook(&mut self) -> Result<(), HookError> {
        let mut inner = self
            .inner
            .lock()
            .map_err(|e| HookError::Other(format!("Could not get lock to inner hook: {}", e)))?;

        let hook = match &inner.hook {
            None => return Err(HookError::NotHooked),
            Some(v) => v,
        };
        ...
        for idx in 0..hook.prelude.len() {
            ...
            *(prelude_ptr.add(idx)) = prelude_byte;
        }
...
impl UserData for Hook {
    fn add_methods<M: mlua::UserDataMethods<Self>>(methods: &mut M) {
        methods.add_method_mut("unhook", |_, this, ()| {
            unsafe {
                this.unhook()
                    .map_err(|e| mlua::Error::RuntimeError(format!("Could not unhook: {:?}", e)))?
            };
    ...
    unsafe {
        ...
        let close_thread_snap_handle = || -> Result<(), anyhow::Error> {
            CloseHandle(thread_snap)
                .map_err(|e| anyhow!("Could not close handle to thread snapshot: {}", e))
        };
        ...
        loop {
            if thread_entry.th32OwnerProcessID == own_process_id
                && thread_entry.th32ThreadID != own_thread_id
            {
