...
pub fn hook_function(
    ...
    debug!(
        "Creating hook on {:#08x} with type {:?} -> {}",
        address, arg_type_names, return_type_name
    );
    ...
    let return_type = match Type::try_from_str(return_type_name.as_str()) {
        ...
        None => {
            return Err(mlua::Error::RuntimeError(format!(
                "return type invalid: type '{}' doesn't exist",
                return_type_name
            )))
        }
    };
    ...
    for arg_type_name in arg_type_names {
        let arg_type = match Type::try_from_str(arg_type_name.as_str()) {
            ...
            None => {
                return Err(mlua::Error::RuntimeError(format!(
                    "argument type invalid: type '{}' doesn't exist",
                    arg_type_name
                )))
            }
        };
        ...
        let hook_closure = move |original_fn: u32, args: u32| {
            ...
            let original_wrapper = match lua.create_function::<_, _, mlua::Value>(
                move |lua, args: MultiValue| {
                    ...
                    for arg_idx in (0..wrapper_argument_types.len()).rev() {
                        ...
                        let mut converted_arg = match lua_to_native(*arg_type, lua_arg) {
                            ...
                            Err(e) => {
                                return Err(mlua::Error::RuntimeError(format!(
                                    "could not converted argument {} into {:?}: {:?}",
                                    arg_idx, *arg_type, e
                                )))
                            }
                        };
                ...
                Err(e) => {
                    ...
                    panic!(
                        "Could not create a wrapper for the original function of a hook: {:?}",
                        e
                    );
                }
            };
            ...
            for i in 0..argument_types.len() {
                ...
                match native_to_lua(&lua, arg_type, *arg_pointer.byte_offset(i as isize * 4)) {
                    ...
                    Err(e) => {
                        warn!(
                            "could not convert {} argument to lua value: {:?}. Panicking...",
                            i, e
                        );
            ...
            let raw_value = match lua_to_native(hook_return_type, &return_value) {
                ...
                Err(e) => {
                    error!(
                        "Could not convert lua hook return value into: {:?}. Panicking...",
                        e
                    );
                    panic!(
                        "Error while converting the return value of a lua hook: {:?}",
                        e
                    );
                }
            };
...
impl NativeFunction {
    ...
    pub fn call(&self, lua: &Lua, args: mlua::MultiValue) -> Result<mlua::Value, mlua::Error> {
        ...
        debug!(
            ...
            self.address, args, self.return_type
        );

        let mut arg_bytes: Vec<u32> = Vec::new();

        for arg in args.iter().rev() {
            let mut arg_byte = unsafe {
                lua_to_native_implied(&arg).map_err(|e| {
                    mlua::Error::RuntimeError(format!(
                        "could not convert lua value into bytes: {}",
                        e.to_string()
                    ))
                })?
            };
        ...
        unsafe {
            ...
            lua_response.map_err(|e| {
                mlua::Error::RuntimeError(format!(
                    "could not convert return value into lua value: {}",
                    e.to_string()
                ))
            })
        }
...
pub fn create_native_function_function(
    ...
    debug!(
        "Creating native function with signature ({:?}) -> {:?}. Calls lua function: {:?}",
        arg_types, return_type, lua_fn
    );
    ...
    let native_closure: Box<dyn FnMut(u32) -> u32> = Box::new(move |args: u32| -> u32 {
        ...
        let return_value = match lua_fn.call::<mlua::Value>(mlua::MultiValue::from_iter(lua_args)) {
            ...
            Err(e) => {
                ...
                panic!(
                    "Lua function in native wrapper threw unexpected error: {:?}",
                    e
                );
            }
        };
        ...
        let native_return_value = unsafe {
            match lua_to_native(lua_ret_type, &return_value) {
                ...
                Err(e) => {
                    warn!(
                        "could not convert lua return value into native value: {:?}. Panicking...",
                        e
                    );
                    panic!(
                        "could not convert lua return value into native value: {:?}",
                        e
                    );
                }
            }
    ...
    unsafe {
        ...
        let closure_wrapper =
            VirtualAlloc(None, 100, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
        ...
        *store_args_in_eax_addr = 0xb8;
        memory_copy(
            &arg_len_in_bytes as *const u32 as u32,
            store_args_in_eax_addr.add(1) as u32,
            4,
        );
        offset += 5;
        ...
        *call_closure_addr = 0xe8u8;
        memory_copy(
            &jmp_delta as *const i32 as u32,
            call_closure_addr.add(1) as u32,
            4,
        );
        offset += 5;
        ...
        *mov_arg_len_in_ecx_addr = 0xb9u8;
        ...
        memory_copy(
            &args_with_data_len as *const u32 as u32,
            mov_arg_len_in_ecx_addr.add(1) as u32,
            4,
        );
        offset += 5;
